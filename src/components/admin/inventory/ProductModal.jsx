...
const ProductModal = ({
  ...
  const handleQuickAdd = async (target) => {
    if (!tempValue) return;
    if (target === "category" && !onAddCategory)
      return alert("Fungsi onAddCategory belum dioper!");
    if (target === "type" && !onAddType)
      return alert("Fungsi onAddType belum dioper!");
    ...
    try {
      if (target === "category") {
        ...
        setForm((prev) => ({ ...prev, category_id: newId }));
        setNewCatMode(false);
      } else {
        const res = await onAddType({ name: tempValue });
        const newId = res.id || res.data?.id;
        setForm((prev) => ({ ...prev, type_id: newId }));
        setNewTypeMode(false);
      }
      setTempValue("");
    } catch (err) {
      console.error("Quick Add Error:", err);
      alert("Gagal menambah data baru. Cek console!");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const inputClass =
    "w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 focus:bg-white transition-all font-medium";
  const labelClass =
    "text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block px-1";
  const quickBtnClass =
      ...
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative group">
                <div
                  className={`w-full h-40 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center transition-all ${form.image ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-slate-50"}`}
                >
                  {form.image ? (
        ...
        <div className="px-8 py-6 bg-slate-50/50 border-t border-slate-50 flex gap-4">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-400 bg-white border border-slate-200"
          >
            Batal
          </button>
          <Button
